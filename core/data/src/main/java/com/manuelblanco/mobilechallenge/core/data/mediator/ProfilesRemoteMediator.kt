...
class ProfilesRemoteMediator @Inject constructor(
     ...
     */
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ProfileEntity>): RemoteKeyEntity? {
        // Get the last page that was retrieved, that contained items.
        // From that last page, get the last item
        return state.pages.lastOrNull {
            it.data.isNotEmpty()
        }?.data?.lastOrNull()?.let { profile ->
            remoteKeysDao.getRemoteKeyByProfileId(profile.profileId)
        }
    }
     ...
     */
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ProfileEntity>): RemoteKeyEntity? {
        // Get the first page that was retrieved, that contained items.
        // From that first page, get the first item
        return state.pages.firstOrNull {
            it.data.isNotEmpty()
        }?.data?.firstOrNull()?.let { profile ->
            remoteKeysDao.getRemoteKeyByProfileId(profile.profileId)
        }
    }
     ...
     */
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ProfileEntity>): RemoteKeyEntity? {
        // The paging library is trying to load data after the anchor position
        // Get the item closest to the anchor position
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.profileId?.let { id ->
    ...
    override suspend fun load(
        ...
        try {
            ...
            transactionProvider.runAsTransaction {
                ...
                val remoteKeys = profiles.map {
                    RemoteKeyEntity(profileId = it.profileId, prevKey = prevKey, currentPage = page, nextKey = nextKey)
                }

                remoteKeysDao.insertAll(remoteKeys)
                profileDao.insertAll(profiles.onEachIndexed { _, profile -> profile.page = page })
            }
